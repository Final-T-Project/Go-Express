  ...
  const handleLogIn = () => {
    if (!value.email.length || !value.password.length) {
      ...
      .then((userCredential) => {
        ...
        alert(
          "YYEYYYY CREDENTIAL ARE CORRECT"
        );
        ...
        if (error.code === "auth/wrong-password") {
          ...
          console.log("----------ERROR----PASSWORD------>" + value.error);
            ...
            if(error.code==="auth/too-many-requests"){
                alert("Your account has been frozed for a moment, you should click on 'Forget my password'")
        ...
        <View style={css.box}>
            ...
            <View style={{justifyContent: 'center',alignItems:'center',marginTop:100}}>
                {!value.emailError?<TextInput keyboardType="email-address"
                        ...
                        style={{backgroundColor:"white",height: 50,fontSize:17,borderColor:'red',borderWidth:3,padding:10,width:330,borderRadius:50,alignItems:'center',}}
                        placeholder="  ✉️   Your Email here..."  onChangeText={(text) => {
                            setValue({ ...value, email: text })}} 
                        />}



                    {!value.passwordError?<TextInput  secureTextEntry={true} 
                        ...
                        style={{backgroundColor:"white",height: 50,fontSize:17,borderColor:'red',borderWidth:3,padding:10,width:330,borderRadius:50,alignItems:'center',marginTop:40}}
                        placeholder="  🔐   Your Password here..."  onChangeText={(text) => setValue({ ...value, password: text })}
                        />}
            </View>
